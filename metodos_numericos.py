...
# Quando pp_kmaisum não é passado, temos que fazer uma predição com Runge-Kutta. Caso contrário, se trata de uma correção
def backward_differentiation_formula_3(lista_pp, delta_t, pp_kmaisum=None):
    flag_predicao = False
    if pp_kmaisum is None:
        pp_kmaisum = runge_kutta_2(lista_pp[2], delta_t)
        flag_predicao = True

    presa = (18 * lista_pp[2].presa - 9 * lista_pp[1].presa + 2 * lista_pp[0].presa + 6 * delta_t * pp_kmaisum.f_presa())/11.0
    predador = (18 * lista_pp[2].predador - 9 * lista_pp[1].predador + 2 * lista_pp[0].predador + 6 * delta_t * pp_kmaisum.f_predador())/11.0
